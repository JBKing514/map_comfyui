import torch
import numpy as np
import io
import matplotlib.pyplot as plt
import comfy.sample
import comfy.model_management
import nodes
import copy
import csv
import os
import folder_paths
from datetime import datetime

# === Global Var ===
GLOBAL_PROJ_MATRIX = None
GLOBAL_PROJ_DIM = 0

class MAP_Pro_Suite:
    HISTORY_CACHE = {}

    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "model": ("MODEL",),
                "seed": ("INT", {"default": 0, "min": 0, "max": 0xffffffffffffffff}),
                "positive": ("CONDITIONING", ),
                "negative": ("CONDITIONING", ),
                "latent_image": ("LATENT", ),
                "sampler_name": (comfy.samplers.KSampler.SAMPLERS, ),
                "scheduler": (comfy.samplers.KSampler.SCHEDULERS, ),
                "denoise": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 1.0, "step": 0.01}),
                
                "operation_mode": (["Analyze (Manual)", "Auto-Tune (Hill Climb)"], ),
                
                # Manual Params
                "steps": ("INT", {"default": 20, "min": 1, "max": 10000}),
                "cfg": ("FLOAT", {"default": 8.0, "min": 0.0, "max": 100.0}),
                
                # Auto-Tune Params
                "tuner_max_steps": ("INT", {"default": 50, "min": 20, "max": 200}),
                "tuner_stride": ("INT", {"default": 5, "min": 1, "max": 20}),
                "tuner_refine_cfg": ("BOOLEAN", {"default": True}),
                
                # Logging
                "save_log_csv": ("BOOLEAN", {"default": True}),
            },
            "optional": {
                # Optional Prompt Inputs for Logging
                "optional_positive_text": ("STRING", {"forceInput": True}),
                "optional_negative_text": ("STRING", {"forceInput": True}),
            }
        }

    RETURN_TYPES = ("LATENT", "IMAGE", "STRING")
    RETURN_NAMES = ("latent", "analysis_plot", "report_text")
    FUNCTION = "execute_map"
    CATEGORY = "MAP_Protocol"

    def execute_map(self, model, seed, positive, negative, latent_image, sampler_name, scheduler, denoise,
                    operation_mode, steps, cfg, tuner_max_steps, tuner_stride, tuner_refine_cfg, 
                    save_log_csv, optional_positive_text=None, optional_negative_text=None):
        
        # Transform seed to int for esay handling
        seed_int = int(seed)

        if operation_mode == "Analyze (Manual)":
            return self.run_manual_analysis(
                model, seed_int, steps, cfg, sampler_name, scheduler, positive, negative, latent_image, denoise
            )
        else:
            return self.run_auto_tuner(
                model, seed_int, steps, cfg, sampler_name, scheduler, positive, negative, latent_image, denoise,
                tuner_max_steps, tuner_stride, tuner_refine_cfg,
                save_log_csv, optional_positive_text, optional_negative_text
            )

    # maual analysis
    def run_manual_analysis(self, model, seed, steps, cfg, sampler, scheduler, pos, neg, lat, denoise):
        print(f"[MAP] Manual Run (Seed={seed})...")
        result_lat, trajectory = self.core_sampling(model, seed, steps, cfg, sampler, scheduler, pos, neg, lat, denoise)
        if len(trajectory) < 5: return (result_lat, self.create_empty_image(), "Insufficient steps")
        coords, c_norm, s_good, q_score, c_raw, ratio = self.calculate_metrics(trajectory)
        delta_q, status_code = 0.0, 0
        if seed in self.HISTORY_CACHE:
            delta_q = q_score - self.HISTORY_CACHE[seed]
            status_code = 1
            print(f"[MAP] Exact Cache Hit. Delta={delta_q:+.2%}")
        self.HISTORY_CACHE[seed] = q_score
        status_str = " (Ref)" if status_code == 0 else f" ({delta_q:+.2%})"
        report = f"MAP Analysis:\nQ: {q_score:.2%}{status_str}\nD: {c_norm:.1%} | S: {s_good:.1%}"
        plot = self.plot_manual(coords, c_norm, s_good, q_score, delta_q, status_code)
        return (result_lat, plot, report)

    # Auto-Tuner 
    def run_auto_tuner(self, model, seed, start_steps, base_cfg, sampler, scheduler, pos, neg, lat, denoise,
                       max_steps, stride, refine_cfg, save_csv, pos_text, neg_text):
        
        print(f"\n[MAP Auto-Tune] Starting Optimization (Seed={seed})")
        print(f"--------------------------------------------------")
        
        history = []
        best_record = None
        
        current_steps = start_steps
        current_cfg = base_cfg
        
        # === Step 1 Hill climb ===
        while current_steps <= max_steps:
            # Print current attempt
            print(f"  > Sampling: Steps={current_steps}, CFG={current_cfg}...", end="", flush=True)
            
            result_lat, trajectory = self.core_sampling(model, seed, current_steps, current_cfg, sampler, scheduler, pos, neg, lat, denoise)
            
            if len(trajectory) >= 5:
                _, _, _, q_score, _, _ = self.calculate_metrics(trajectory)
                
                # print result
                print(f" Done. Q={q_score:.2%}")
                
                record = {'steps': current_steps, 'cfg': current_cfg, 'q': q_score, 'lat': result_lat}
                history.append(record)
                
                if best_record is None:
                    best_record = record
                    print(f"    [NEW BEST] Initial baseline.")
                elif q_score >= best_record['q'] * 0.995: # allow small tolerance
                    diff = q_score - best_record['q']
                    best_record = record
                    print(f"    [NEW BEST] Improved by {diff:+.2%}")
                else:
                    print(f"    [STOP] Peak detected. Quality dropped from {best_record['q']:.2%} to {q_score:.2%}")
                    break 
            else:
                print(" Failed (Trajectory too short).")
            
            current_steps += stride
            comfy.model_management.throw_exception_if_processing_interrupted()

        # === step 2 CFG fine tune ===
        if refine_cfg and best_record:
            print(f"\n[MAP Auto-Tune] Refining CFG around {best_record['cfg']}...")
            opt_steps = best_record['steps']
            for delta in [-0.5, 0.5]:
                test_cfg = base_cfg + delta
                
                print(f"  > Sampling: Steps={opt_steps}, CFG={test_cfg}...", end="", flush=True)
                
                result_lat, trajectory = self.core_sampling(model, seed, opt_steps, test_cfg, sampler, scheduler, pos, neg, lat, denoise)
                if len(trajectory) >= 5:
                    _, _, _, q_score, _, _ = self.calculate_metrics(trajectory)
                    print(f" Done. Q={q_score:.2%}")
                    
                    if q_score > best_record['q']:
                        print(f"    [NEW BEST] CFG improved quality!")
                        best_record = {'steps': opt_steps, 'cfg': test_cfg, 'q': q_score, 'lat': result_lat}
                        history.append(best_record)
                    else:
                        print(f"    No improvement.")

        # === step 3 cache and csv log ===
        if best_record:
            # 1. synch cache
            self.HISTORY_CACHE[seed] = best_record['q']
            
            # 2. csv log
            if save_csv:
                self.save_to_csv(seed, best_record, pos_text, neg_text, sampler, scheduler)

        # generate final report and plot
        report = (
            f"MAP Optimization Complete:\n"
            f"------------------------\n"
            f"Seed: {seed}\n"
            f"Best Steps: {best_record['steps']}\n"
            f"Best CFG: {best_record['cfg']}\n"
            f"Max Quality: {best_record['q']:.2%}\n"
        )
        print(f"--------------------------------------------------")
        print(f"[MAP Auto-Tune] Finished. Best Q={best_record['q']:.2%} @ Steps={best_record['steps']}\n")
        
        plot = self.plot_tuning_curve(history, best_record)
        return (best_record['lat'], plot, report)

    # csv logging function
    def save_to_csv(self, seed, record, pos_text, neg_text, sampler, scheduler):
        try:
            output_dir = folder_paths.get_output_directory()
            csv_path = os.path.join(output_dir, "MAP_Tuning_Log.csv")
            
            file_exists = os.path.isfile(csv_path)
            
            # data row
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            pos_content = pos_text if pos_text else "N/A"
            neg_content = neg_text if neg_text else "N/A"
            
            # if content too long, truncate
            if len(pos_content) > 200: pos_content = pos_content[:200] + "..."
            
            row = [
                timestamp,
                str(seed),
                f"{record['q']:.4f}", # Q Score
                str(record['steps']),
                str(record['cfg']),
                sampler,
                scheduler,
                pos_content,
                # neg_content # negative prompt is not logged to reduce csv size
            ]
            
            with open(csv_path, mode='a', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                # header
                if not file_exists:
                    writer.writerow(["Timestamp", "Seed", "Q_Score", "Best_Steps", "Best_CFG", "Sampler", "Scheduler", "Prompt"])
                # data row
                writer.writerow(row)
                
            print(f"[MAP] Log saved to {csv_path}")
            
        except Exception as e:
            print(f"[MAP] Failed to save CSV: {e}")
    # core sampling with trajectory capture
    def core_sampling(self, model, seed, steps, cfg, sampler, scheduler, pos, neg, lat, denoise):
        traj = []
        def cb(s, x0, x, t):
            try: traj.append(x[0].detach().cpu().flatten())
            except: pass
        orig = comfy.sample.sample
        comfy.sample.sample = lambda *a, **k: orig(*a, **{**k, 'callback': cb})
        try:
            res = nodes.common_ksampler(model, seed, steps, cfg, sampler, scheduler, pos, neg, lat, denoise)
            return res[0], traj
        except Exception as e:
            print(f"Sampling Error: {e}")
            return lat, []
        finally:
            comfy.sample.sample = orig

    def calculate_metrics(self, traj):
        X = torch.stack(traj).float()
        global GLOBAL_PROJ_MATRIX, GLOBAL_PROJ_DIM
        if GLOBAL_PROJ_MATRIX is None or GLOBAL_PROJ_DIM != X.shape[1]:
            P = torch.randn(X.shape[1], 2)
            GLOBAL_PROJ_MATRIX = P / torch.norm(P, dim=0, keepdim=True)
            GLOBAL_PROJ_DIM = X.shape[1]
        coords = torch.mm(X - X[0], GLOBAL_PROJ_MATRIX).numpy()
        c_raw = np.max(np.linalg.norm(coords, axis=1))
        c_norm = c_raw / (c_raw + 5.0)
        vels = np.linalg.norm(coords[1:] - coords[:-1], axis=1)
        tail_len = max(3, int(len(vels)*0.2))
        ratio = np.mean(vels[-tail_len:]) / (np.mean(vels)+1e-8)
        s_good = np.exp(-5.0 * ratio)
        return coords, c_norm, s_good, c_norm*s_good, c_raw, ratio

    def plot_manual(self, coords, c, s, q, delta_q, status_code):
        plt.figure(figsize=(10, 12), dpi=100)
        gs = plt.GridSpec(5, 1)
        ax_plot = plt.subplot(gs[0:4, 0])
        ax_text = plt.subplot(gs[4, 0])
        x, y = coords[:, 0], coords[:, 1]
        ax_plot.plot(x, y, color='gray', alpha=0.5, linestyle='--')
        ax_plot.scatter(x, y, c=np.arange(len(x)), cmap='coolwarm', s=100, edgecolors='black', zorder=10)
        ax_plot.text(x[0], y[0], ' Start', va='bottom')
        ax_plot.text(x[-1], y[-1], ' End', va='top', fontweight='bold')
        ax_plot.axis('equal')
        ax_plot.grid(True, alpha=0.3)
        ax_plot.set_title(f"MAP Analysis (Q={q:.1%})", fontsize=14, fontweight='bold', color='#333')
        if status_code == 0:
            ax_plot.text(0.5, 0.05, "Reference Established\nFix seed to tune.", transform=ax_plot.transAxes, fontsize=12, fontweight='bold', ha='center', bbox=dict(fc="#f39c12", alpha=0.8))
        else:
            color = "#27ae60" if delta_q > 0 else "#c0392b"
            sign = "+" if delta_q > 0 else ""
            txt = f"{sign}{delta_q:.1%} Change"
            ax_plot.text(0.95, 0.95, txt, transform=ax_plot.transAxes, fontsize=24, fontweight='bold', color=color, ha='right', va='top', bbox=dict(boxstyle="round,pad=0.3", fc="white", ec=color, lw=2, alpha=0.9))
        ax_text.axis('off')
        self._draw_bar(ax_text, 0.8, "Depth:", c, '#3498db')
        self._draw_bar(ax_text, 0.5, "Stability:", s, '#9b59b6')
        self._draw_bar(ax_text, 0.2, "Quality:", q, '#34495e')
        return self._plt_to_tensor()

    def plot_tuning_curve(self, history, best):
        steps = [h['steps'] for h in history]
        qs = [h['q'] for h in history]
        plt.figure(figsize=(10, 6), dpi=100)
        plt.plot(steps, qs, marker='o', linestyle='-', color='#3498db', linewidth=2)
        plt.scatter(best['steps'], best['q'], s=200, c='#2ecc71', edgecolors='black', zorder=10)
        plt.text(best['steps'], best['q'] + 0.01, f" Optimal\n Step {best['steps']}", ha='center', va='bottom', color='#27ae60', fontweight='bold')
        plt.title("MAP Auto-Tuner Optimization", fontsize=14, fontweight='bold')
        plt.xlabel("Steps")
        plt.ylabel("Quality Score")
        plt.grid(True, alpha=0.3)
        return self._plt_to_tensor()

    def _draw_bar(self, ax, y, label, val, col):
        ax.text(0.1, y, label, fontsize=12, va='center', ha='right', color='#555')
        ax.add_patch(plt.Rectangle((0.15, y-0.1), 0.7, 0.2, color='#eee', transform=ax.transAxes))
        ax.add_patch(plt.Rectangle((0.15, y-0.1), 0.7*min(val,1), 0.2, color=col, transform=ax.transAxes))
        ax.text(0.87, y, f"{val:.1%}", fontsize=12, va='center', fontweight='bold', color='#333')

    def _plt_to_tensor(self):
        buf = io.BytesIO()
        plt.savefig(buf, format='png', bbox_inches='tight')
        plt.close()
        buf.seek(0)
        import PIL.Image
        return torch.from_numpy(np.array(PIL.Image.open(buf).convert("RGB")).astype(np.float32) / 255.0)[None,]

    def create_empty_image(self): return torch.zeros((1, 512, 512, 3))